    ...
    def trans_i(self, p, msgs):
        if len(msgs) == 0:
            ...
            if msg.content == p.UID:
                p.output("status", "leader")
            elif msg.content > p.UID:
                p.set("send", msg)
                if not p.has("decided"):
                    p.set("decided", None)
                    p.output("status", "non-leader")
            else:
                p.set("send",  None)
        if self.r == p.process.state['n']: p.terminate()
...
class AsyncLCR(Asynchronous_Algorithm):
    ...
    def msgs_i(self, p, verbose=False):
        if not p.has("sends"):
            p.set("sends",  [Message(self, p.UID)])
        ...
        if p.state["status"] == "leader":
            ...
            p.terminate()
            return
        while len(p.get("sends")) > 0:
            msg = p.get("sends").pop()
            p.send_msg(msg, p.out_nbrs[-1])
            if verbose:
                print p,"sends",msg
            if p.has("terminate_after_send"):
                p.terminate()

    def trans_i(self, p, verbose=False):
        msgs = p.get_msgs()
        if p.get('sends') is None:
            p.set('sends', [Message(self, p.UID)])
        ...
        while len(msgs) > 0:
            ...
            if isinstance(msg, AsyncLCR.Leader_Declaration):
                p.get("sends").append(msg)
                p.set("terminate_after_send", None)
                return
            elif msg.content == p.UID:
                p.output("status", "leader")
            elif msg.content > p.UID:
...
class SynchFloodMax(Synchronous_Algorithm):
    ...
    """
    def msgs_i(self,p):
        if self.r < p.get( "diam"):
            if not p.has( "send"):
                p.set( "send",  Message(self, p.UID))
            p.send_msg(p.get( "send"))

    def trans_i(self, p, msgs, verbose=False):
        if verbose:
            print p, "received", p.in_channel
        seen_uids = msgs + [p.get( "send")]
        p.set( "send",  max(seen_uids, key = lambda m: m.content))

        if self.r == p.get( 'diam'):
            if p.get( "send").content == p.UID:
                p.output("status", "leader")
                p.terminate()
            else:
                p.output("status", "non-leader")
...
class SynchHS(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        ...
        if not p.has( "send+"):
            p.set('send+', Message(self, plus_msg))

        if not p.has( "send-"):
            p.set('send-', Message(self, minus_msg))
    ...
    def trans_i(self, p, msgs):
        ...
        for msg in msgs:
            if isinstance(msg, SynchHS.Leader_Declaration):
                p.output("status", "non-leader") #Terminates next msgs_i
                return
            ...
            if msg.author == p.out_nbrs[0]:
                if flag == 'out':
                    if val > p.UID and hopcount > 1:
                        p.set( 'send+', Message(self, (val, flag, hopcount-1)))
                    if val > p.UID and hopcount == 1:
                        p.set( 'send-', Message(self, (val, 'in', 1)))
                    if val == p.UID:
                        return p.output("status", "leader") #Terminates next msgs_i
                if flag == 'in':
                    if val != p.UID:
                        p.set( 'send+', msg)
                    else:
        ...
        if left_msg_returned and right_msg_returned:
            #Start next phase
            next_phase = p.get( 'phase') + 1
            p.set( 'phase', next_phase)
            p.set( 'send+', Message(self, (p.UID, "out", 2**next_phase)))
            p.set( 'send-', Message(self, (p.UID, "out", 2**next_phase)))
...
class SynchTimeSlice(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        ...
        elif self.r == (p.UID-1)*p.state['n']+1 and not p.has( "decided"): # check if logic is correct for this
            p.set( 'decided', None)
            p.output("status", "leader")
    ...
    def trans_i(self, p, msgs):
        if len(msgs) > 0:
            ...
            if (self.r - 1)/p.state['n'] == msg.content-1 and not p.has("decided"):
                p.set( 'decided', None)
                p.output("status", "non-leader")
                p.set("send", msg)
    ...
    def msgs_i(self, p):
        ...
        for i in reversed(range(len(queue))):
            ...
            if msg.content["TTS"] <= 0:
                if msg.content['UID'] == p.get( "smallest_uid"):
                    p.send_msg(msg, p.out_nbrs[-1])
                queue.pop(i)
    ...
    def trans_i(self, p, msgs):
        ...
        for msg in msgs:
            if isinstance(msg, SynchVariableSpeeds.Leader_Declaration):
                return p.output('status', 'non-leader') #Terminates next msgs_i
            if msg.content["UID"] == p.UID:
                return p.output('status', 'leader') #Terminates next msgs_i
            if msg.content["UID"] < p.get( "smallest_uid"):
                p.set( "smallest_uid", msg.content["UID"])
...
class SynchBFS(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        if self.is_i0(p) and self.r == 1:
            p.output("parent",  None)
            p.send_msg(SynchBFS.Search(self, p.process))
        if p.has( "recently_marked"):
            p.send_msg(SynchBFS.Search(self, p.process))
            p.delete("recently_marked")
    def trans_i(self, p, msgs):
        if len(msgs)> 0:
            if "parent" not in p.state:
                p.output("parent",  msgs[0].content)
                p.set( "recently_marked",  True)
                return
        if "parent" in p.state:
            if p.has( "recently_marked"): p.delete("recently_marked")
            p.terminate()
...
class SynchBFSAck(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        if self.is_i0(p) and self.r == 1:
            p.output("parent",  None)
            p.set( "recently_marked",  True)
            p.send_msg(SynchBFSAck.Search(self, p.process))
        elif p.has( "recently_marked"):
            p.send_msg(SynchBFSAck.Search(self, p.process))
            p.send_msg(SynchBFSAck.AckParent(self, p.process), p.state['parent'] )
...
class SynchConvergecast(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        if not self.is_root(p):
            if self.r == 1:
                p.set( 'send',  self.initial_msg_to_parent(p))
            if p.get( 'send') is not None:
                p.send_msg(p.get( 'send'), p.state['parent'])
                p.set( 'send',  None)
    def trans_i(self, p, msgs):
        ...
        if self.is_root(p):
            ...
            if len (msgs) > 0:
                p.set( 'send', self.trans_msg_to_parent(p, msgs) )
            else:
                p.terminate()
...
class AsynchConvergecast(Asynchronous_Algorithm):
    ...
    def trans_i(self, p, msgs):
        ...
        if not p.has( 'reports'):
            p.set( 'reports', [])
        p.increment('reports', msgs)
        if len(p.state['children']) == len(p.get( 'reports')):
            if self.is_root(p):
                self.trans_root(p, p.get( 'reports'))
                self.output_root(p)
                p.terminate()
            elif len(p.state['children']) != 0:
                trans_msg = self.trans_msg_to_parent(p, p.get( 'reports'))
                p.set( 'send', trans_msg)
...
def _converge_height(Convergecast, name):
    class _ConvergeHeight(Convergecast):
        ...
        def trans_root(self, p, msgs):      #Updates height
            p.set( 'height', max(msgs))
        def output_root(self, p):           #Decides height
            p.output('height', p.get( 'height')) 
        def initial_msg_to_parent(self, p):
...
class SynchBroadcast(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        ...
        if p.state['parent'] is not None:
            if p.get( 'send') is not None:
                p.send_msg(Message(self,p.get( 'send')), p.state['children'])
                p.set( 'send',  None)
                p.terminate()
    def trans_i(self, p, msgs):
        msgs = [m.content for m in msgs]
...
class SynchLubyMIS(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        if self.r == 1:
            p.set( 'rem_nbrs', p.out_nbrs[:])
            p.set( 'status', 'unknown')

        if self.r%3 == 1: 
            p.set('val', random.randint(0,p.state['n'] **4 ))
            p.send_msg( Message(self, p.get( 'val')), p.get( 'rem_nbrs') )
        if self.r%3 == 2:
            if p.get('status') == 'winner':
                p.send_msg( Message(self, 'winner') , p.get( 'rem_nbrs') )
        if self.r%3 == 0:
             if p.get('status') == 'loser':
                p.send_msg( Message(self, 'loser') , p.get( 'rem_nbrs') )           
    def trans_i(self, p, msgs):
        values = [m.content for m in msgs]
        if self.r%3 ==1:
            if len(values) == 0 or p.get( 'val') > max(values):
                p.set('status', 'winner')
                p.output('MIS', True)
        if self.r%3 ==2:
            if 'winner' in values:
                ...
                p.output('MIS', False)
        if self.r%3 == 0:
            rem_nbrs = p.get( 'rem_nbrs')
            for m in msgs:
                if m.content == 'loser':
                    rem_nbrs.remove(m.author)
            p.set('rem_nbrs', rem_nbrs)
            if p.get( 'status') in ['winner', 'loser']:
                p.terminate()
...
class SynchBellmanFord(Synchronous_Algorithm):
    ...
    def msgs_i(self, p):
        if self.r == 1:
            p.set('SP', p.state['nbr_dist'])
            p.set('send', True)
        
        if p.get('send') == True:
            p.send_msg(Message(self, p.get( 'SP')))

        if self.r == p.state['n']:
            p.output('SP', p.get( 'SP'))
            p.terminate()
